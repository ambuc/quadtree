...
mod query_tests {
    ...
    fn query_empty() {
        ...
        let mut iter = qt.query(((0, 0), (4, 4)).into());
    ...
    fn query_on_point() {
        ...
        qt.insert(((0, 0), (1, 1)).into(), 49);

        // Requesting a region which does contain '49'.
        let mut iter1 = qt.query(((0, 0), (1, 1)).into());
        ...
        let mut iter2 = qt.query(((0, 1), (1, 1)).into());
        debug_assert_eq!(iter2.next(), None);

        let mut iter3 = qt.query(((1, 0), (1, 1)).into());
        debug_assert_eq!(iter3.next(), None);

        let mut iter4 = qt.query(((1, 1), (1, 1)).into());
    ...
    fn query_in_region() {
        ...
        debug_assert!(unordered_elements_are(
            qt.query(((3, 3), (1, 1)).into()).map(|e| e.value_ref()),
            vec![&10, &55],
        ));

        debug_assert!(unordered_elements_are(
            qt.query(((3, 3), (3, 3)).into()).map(|e| e.value_ref()),
            vec![&10, &55],
        ));

        debug_assert!(unordered_elements_are(
            qt.query(((0, 0), (6, 6)).into()).map(|e| e.value_ref()),
            vec![&10, &55],
        ));

        debug_assert!(unordered_elements_are(
            qt.query(((2, 2), (6, 6)).into()).map(|e| e.value_ref()),
            vec![&10, &55],
        ));

        debug_assert!(unordered_elements_are(
            qt.query(((2, 2), (2, 2)).into()).map(|e| e.value_ref()),
            vec![&10, &55],
        ));
    }
    ...
    fn query_strict_in_region() {
        ...
        // Queries which turn up empty:
        debug_assert_eq!(qt.query_strict(((1, 1), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((0, 0), (2, 2)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((0, 0), (6, 2)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((0, 0), (2, 6)).into()).next(), None);

        // Queries which capture portions of #10 but not enough.
        debug_assert_eq!(qt.query_strict(((2, 2), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((2, 3), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((3, 2), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((2, 2), (2, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((2, 2), (1, 2)).into()).next(), None);

        // Queries which capture portions of #55 but not enough.
        debug_assert_eq!(qt.query_strict(((3, 4), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((4, 3), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((4, 4), (1, 1)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((4, 3), (1, 2)).into()).next(), None);
        debug_assert_eq!(qt.query_strict(((3, 4), (2, 2)).into()).next(), None);

        // Queries which capture portions of both #10 and #55. but still aren't enough

        debug_assert_eq!(qt.query_strict(((3, 3), (1, 1)).into()).next(), None);

        // Queries which contain one of the other:
        debug_assert_eq!(
            qt.query_strict(((3, 3), (2, 2)).into())
                .next()
                .unwrap()
                .value_ref(),
            &55
        ...
        debug_assert!(unordered_elements_are(
            qt.query_strict(((0, 0), (6, 6)).into())
                .map(|e| e.value_ref()),
            vec![&10, &55]
        ));
        debug_assert!(unordered_elements_are(
            qt.query_strict(((2, 2), (6, 6)).into())
                .map(|e| e.value_ref()),
            vec![&10, &55]
        ));
    }
    ...
    fn query_exhibiting_collection() {
        ...
        qt.insert(((0, 0), (2, 2)).into(), 1.234);

        let mut query_obj = qt.query(((0, 0), (1, 1)).into());
